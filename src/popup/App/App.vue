        ...
        >
            ...
            <el-table-column label="规则" width="400">
                <template slot-scope="scope">
                    <el-input v-model="scope.row.rule" placeholder="请输入内容" @change="handleInputChange(scope.row)"> </el-input>
                </template>
            </el-table-column>
            <el-table-column label="状态" width="100">
                <template slot-scope="scope">
                    <el-tag :type="scope.row.enabled ? 'success' : 'danger'" disable-transitions>{{ scope.row.enabled ? '启用' : '禁用' }}</el-tag>
                </template>
            </el-table-column>
            <el-table-column label="操作" width="150">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.enabled" size="mini" @click="toggleStatus([scope.row], false)">禁用 </el-button>
